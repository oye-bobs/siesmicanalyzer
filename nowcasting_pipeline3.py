...
# --- Machine Learning Specific Imports (moved up for clarity) ---
...
if 'df_features' in locals() and not df_features.empty:
    ...
    if len(X) == 0:
        ...
        # 5.3. Model Selection and Training (use resampled data, remove class_weight='balanced')
        print("\nTraining RandomForestClassifier on SMOTE-resampled data...")
        model = RandomForestClassifier(n_estimators=100, random_state=42) 
        model.fit(X_train_resampled, y_train_resampled) # Use resampled data for training
        print("Model training complete.")

        # 5.4. Evaluation (always on the original, UNRESAMPLED test set)
