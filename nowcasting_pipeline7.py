...
LON_MIN = -125.0
LON_MAX = -114.0
GRID_SIZE_DEG = 1.0 # 1 degree by 1 degree grid
...
min_events_per_cell = 5 # Minimum events in a cell for feature calculation
...
if len(df_catalog) == 0:
    ...
    while current_window_start + timedelta(days=window_size_days + prediction_horizon_days) <= end_date_catalog:
        ...
        row_features["seismicity_rate_region"] = len(df_window) # Renamed to avoid conflict
    ...
    # Handle NaNs from inactive grid cells
    # We will fill NaNs for the spatial features with 0, assuming no activity means 0 seismicity/b-value is irrelevant
...
# --- 5. Machine Learning Model Training and Evaluation (XGBoost with Refined Tuning) ---
print("\n--- Step 5: Machine Learning Model Training and Evaluation ---")

if 'df_features' in locals() and not df_features.empty:
    # 5.1. Prepare Data for ML - NOW INCLUDES SPATIAL FEATURES
    ...
    if len(X) == 0:
        ...
        # --- Step 6: Hyperparameter Tuning with GridSearchCV for XGBoost (Limited Grid for Spatial Features) ---
        print("\n--- Step 6: Hyperparameter Tuning with GridSearchCV for XGBoost (LIMITED GRID FOR SPATIAL FEATURES) ---")
        
        # Given the increased number of features, we will use a more limited param_grid
        # to prevent extremely long runtimes. We'll start with parameters close to the best found.
        param_grid = {
            'n_estimators': [200, 250], # Keep it tight around the previous best
        ...
        # This grid will test 2 * 2 * 2 * 2 * 2 * 2 = 64 combinations.
        # Still 64 * 5 = 320 models with CV.
        ...
        print("Starting GridSearchCV for XGBoost with spatial features...")
