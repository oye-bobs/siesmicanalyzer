...
if 'df_features' in locals() and not df_features.empty:
    ...
    if len(X) == 0:
        ...
        # --- Step 6: Hyperparameter Tuning with GridSearchCV for XGBoost (REFINED GRID) ---
        print("\n--- Step 6: Hyperparameter Tuning with GridSearchCV for XGBoost (REFINED GRID) ---")
        
        # Define the parameter grid for XGBoost (REFINED)
        param_grid = {
            'n_estimators': [150, 200, 250, 300], # Finer tuning around 200, and slightly higher
            'learning_rate': [0.07, 0.1, 0.15], # Finer tuning around 0.1
            'max_depth': [6, 7, 8], # Finer tuning around 7
            'subsample': [0.6, 0.7, 0.8], # Finer tuning around 0.7
            'colsample_bytree': [0.6, 0.7, 0.8], # Finer tuning around 0.7
            'gamma': [0, 0.05, 0.1] # Finer tuning around 0
        }
        ...
        print("Starting GridSearchCV for XGBoost with refined parameters...")
